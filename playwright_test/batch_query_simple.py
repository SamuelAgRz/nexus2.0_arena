...
Captures only: original question, summarizer result, and total time spent.
...
Usage:
    .venv\Scripts\python.exe playwright_test/batch_query_simple.py
...
CSV_FILE = Path(__file__).parent / "questions_ontology_test_Colombia.csv"
...
CSV_COLUMNS = ["index", "question", "total_runtime_seconds", "agent_timings", "summarizer", "error"]


def parse_agent_timings(text: str) -> list[dict]:
    """Extract source name, request time, and response time for every agent block."""
    pattern = r"Source:\s*(.+?)\n\nRequest Time:\s*(.+?)\n\nResponse Time:\s*(.+?)\n\n"
    return [
        {"source": m.group(1).strip(), "request_time": m.group(2).strip(), "response_time": m.group(3).strip()}
...
async def run_question(
    ...
    async with semaphore:
        ...
        try:
            ...
            agent_timings = parse_agent_timings(response_text)
            ...
            timings_str = "\n".join(
                f"{t['source']}: {t['request_time']} → {t['response_time']}"
